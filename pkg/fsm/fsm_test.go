...
func TestSpecificCallbacks(t *testing.T) {
	...
	fsm := NewFSM(
		...
		Callbacks{
			BeforeEventOrState("run"): func(e *Event) {
				beforeEvent = true
			},
	...
	fsm = NewFSM(
		...
		Callbacks{
			"before_run": func(e *Event) {
				...
				// transition checks
				if len(e.Args) == 0 {
					// Must be concurrent so the go_test may pass when we add a mutex that synchronizes
					// calls to Event(...). It will then fail as an inappropriate transition as we
					// have changed state.
					go func() {
						if err := fsm.Event("run", "second run"); err != nil {
							fmt.Println(err)
							wg.Done() // It should fail, and then we unfreeze the go_test.
						}
					}()
...
func TestFSM_Last(t *testing.T) {
	...
	if fsm.Last() != "closed" && fsm.Current() != "open" {
		t.Failed()
	}
}
